...
| 1.2.0.0 | Y-Auswahl im Service repariert: In der Leseansicht verwirft Power BI persistProperties-Schreibversuche, wodurch der Selector wirkungslos blieb. Die Auswahl wird jetzt lokal im Visual gehalten und sofort gezeichnet; die Persistenz in die Report-Datei erfolgt zusätzlich, wo erlaubt (Desktop/Bearbeitungsmodus). In der Leseansicht gilt die Auswahl damit pro Sitzung. |
| 1.3.0.0 | X-Facetten-Auswahl im Visual: Chip-Leiste unter der Y-Auswahl blendet einzelne Indikator-Facetten ein/aus (ausgeblendete Chips gedimmt und durchgestrichen). Zustand lokal gehalten (funktioniert in der Service-Leseansicht) und zusätzlich persistiert, wo erlaubt; abschaltbar unter Facetten → X-Facetten-Auswahl im Visual. |
